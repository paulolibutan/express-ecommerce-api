  ...
  User.findOne({ email })
    .then((user) => {
      ...
      if (!user.emailConfirmed) {
        ...
        try {
          ...
          return res.status(400).send({
            error:
              "Email is not yet confirmed. We have resent the confirmation link to your email.",
          });
        } catch (error) {
          console.error(error);
          return res.status(500).send({ error: "Error sending email" });
      ...
      if (isPasswordCorrect) {
        return res
          .status(200)
          .send({ accessToken: auth.createAccessToken(user) });
      } else {
        return res
          .status(401)
...
module.exports.getUserDetails = (req, res) => {
  User.findById(req.user.id, { password: 0 })
    ...
    .catch((err) => {
      ...
      return res
        .status(500)
        .send({ error: "Error in retrieving user details" });
    });
};
  ...
  User.findOne({ email })
    .then((user) => {
      ...
      const restPasswordLink =
        req.protocol + "://" + req.get("host") + req.originalUrl + token;

      const mailOptions = {
        from: process.env.CONFIRMATION_EMAIL_FROM,
        to: email,
        subject: "Ecommerce App: Password Reset Request",
        text: `You have requested to reset your password. Click the following link to continue: ${restPasswordLink}`,
      };
      ...
      try {
        ...
        return res
          .status(200)
          .send({ message: "Email sent with password reset instructions" });
      } catch (error) {
        console.error(error);
        return res.status(500).send({ error: "Error sending email" });
      }
